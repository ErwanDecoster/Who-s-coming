...
// data.value.user = await useFetch(
...
<template>
	<div class="container">
			...
			<div class="grid gap-1">
				...
				<div class="grid gap-1" v-if="data.event.needs.length">
					<div 
						...
						:key="need.id_need"
						class="flex gap-1 rounded-sm items-center"
					>
            <input type="checkbox" :name="need.id_need" :id="need.id_need">
            <label class="text-base text-black-300 first-letter:uppercase grow flex justify-between" :for="need.id_need">
              {{ need.label }}
              <span>
                {{ need.actual_number }} / {{ need.min_required_number }}
              </span>
            </label>
          </div>
				</div>
      ...
      <NuxtLink :to="`/events/${$route.params.id}-${toSlug($route.params.name.toString())}/invites`" class="secondary">Liste des invités</NuxtLink>
			<button :to="`/events/${$route.params.id}-${toSlug($route.params.name.toString())}/edit`" class="primary">Comfirmer l'invitation</button>
		</template>
	</div>
</template>
